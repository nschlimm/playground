...
public class Basic {
	...
	/*
	 * Generate buffer with random contents Writes buffer to file using a CompletionHandler to consume the result of
	 * each write operation Reads file to EOF to a new buffer using a CompletionHandler to consume the result of each
	...
	static void testUsingCompletionHandlers(AsynchronousFileChannel ch) throws IOException {
		...
		ByteBuffer dst = (rand.nextBoolean()) ? ByteBuffer.allocateDirect(src.capacity()) : ByteBuffer.allocate(src
				.capacity());
		readAll(ch, dst, 0L);
	...
	/*
	 * Generate buffer with random contents Writes buffer to file, invoking the Future's get method to wait for each
	...
	static void writeFully(final AsynchronousFileChannel ch, final ByteBuffer src, long position) {
		...
		ch.write(src, position, position, new CompletionHandler<Integer, Long>() {
			public void completed(Integer result, Long position) {
				...
				if (src.hasRemaining()) {
					long p = position + n;
					ch.write(src, p, p, this); // läuft im aktuellen thread deswegen muss position nicht synchronisiert
												// werden
